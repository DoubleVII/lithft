...
def setup(
    ...
    out_dir: Path = Path("out/finetune"),
    ...
    eval: EvalArgs = EvalArgs(interval=100000, max_new_tokens=100, max_iters=100),
    ...
    logger = choose_logger(
        logger_name,
        out_dir,
        name=f"finetune-{config.architectures[0]}",
        resume=resume,
...
def fit(
    ...
    validate(
        ...
        data,
    ) if val_dataloader is not None else validate(
        fabric,
        model,
        train_dataloader,
        tokenizer,
