...
from litgpt.data import DataModule, TinyLlama, PackedData
...
# hack lightning requirement check
if not L.pytorch.loggers.wandb._WANDB_AVAILABLE:
    L.pytorch.loggers.wandb._WANDB_AVAILABLE = True
...
def setup(
    ...
    if model_config is not None and initial_checkpoint_dir is not None:
        raise ValueError(
            "Only one of `initial_checkpoint_dir` or `model_config` can be set."
        )
    elif model_config is None and initial_checkpoint_dir is None:
        raise ValueError("Please specify --initial_checkpoint_dir or --model_config")
    config = AutoConfig.from_pretrained(
        model_config if initial_checkpoint_dir is None else initial_checkpoint_dir
    )
    ...
    logger = choose_logger(
        logger_name,
        out_dir,
        name=f"pretrain-{config.architectures[0]}",
        resume=resume,
        log_interval=train.log_interval,
    )

    if devices > 1:
        # strategy = FSDPStrategy(state_dict_type="full", sharding_strategy="HYBRID_SHARD")
        strategy = DeepSpeedStrategy(
            stage=2 if not zero3 else 3,
            overlap_comm=False if not zero3 else True, # see https://github.com/microsoft/DeepSpeed/issues/4298
        )
        # pref: zero2 items_per_sec 165158.46472 vs zero3 items_per_sec 175697.35568

        strategy.config["gradient_clipping"] = 1.0
    else:
        strategy = "auto"
    fabric = L.Fabric(
        devices=devices, strategy=strategy, precision="bf16-mixed", loggers=[logger]
    )
...
def main(
    ...
    with fabric.init_module(empty_init=True):
        ...
        dschf = HfDeepSpeedConfig(fabric.strategy.config)
        if initial_checkpoint_dir is not None:
            model = AutoModelForCausalLM.from_pretrained(initial_checkpoint_dir)
        else:
...
def fit(
    ...
    # sanity check
    (
        validate(fabric, model, val_dataloader, max_iters=2)
        if val_dataloader is not None
        else validate(fabric, model, train_dataloader, max_iters=2)
    )
    throughput = ThroughputMonitor(fabric, window_size=5)

    with torch.device("meta"):
        meta_model = AutoModelForCausalLM.from_config(model.config)
        meta_model.config.n_embd = meta_model.config.hidden_size
        meta_model.config.n_layer = meta_model.config.num_hidden_layers
        meta_model.max_seq_length = model.max_seq_length
        # x = torch.randint(0, 1, (train.micro_batch_size, model.max_seq_length))
        # model_fwd = lambda: meta_model(x).logits
        # model_loss = lambda y: chunked_cross_entropy(y, x, chunk_size=0)
        measured_flops = (
            estimate_flops(meta_model, training=True) * train.micro_batch_size
        )
        fabric.print(
...
def get_dataloaders(
    fabric: L.Fabric, data: DataModule, tokenizer, train: TrainArgs, block_size: int
    ...
    data.setup()
    # TODO
    train_dataloader = data.train_dataloader(fabric)
    val_dataloader = data.val_dataloader(fabric)
